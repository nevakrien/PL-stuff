...
REPL_BIN := pl_repl
REPL_SRCS := repl.c frontend.c vm.c verification.c ir.c
TEST_BINS := $(VM_IR_TEST_BIN) $(VERIFICATION_TEST_BIN) $(FRONTEND_TEST_BIN)

.PHONY: all test clean

all: $(TEST_BINS) $(REPL_BIN)
...
$(REPL_BIN): $(REPL_SRCS) $(HEADERS)
	$(CC) $(CFLAGS) $(REPL_SRCS) -o $@

test: $(TEST_BINS) $(REPL_BIN)
	./$(VM_IR_TEST_BIN)
	./$(VERIFICATION_TEST_BIN)
	./$(FRONTEND_TEST_BIN)
	sh tests/repl_test.sh

clean:
	rm -f $(TEST_BINS) $(REPL_BIN)
